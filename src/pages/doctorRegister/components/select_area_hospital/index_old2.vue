<template>
  <div class="choose-wrapper">
    <div class="chose_layer chose_room" v-show="this.showChoose">
        ...
        <div class="city-wrapper all" v-if="firstList && firstList.length>0">
          <p class="title">所有城市</p>
          <dl>
            <dd class="flex-space-between" v-for="(item,index) in firstList" @click="chooseFirst(index)" :key="index">
              <span>{{item}}</span>
    ...
    <div class="chose_layer chose_room" v-show="showChooseSecond">
        ...
        <span class="flex1">市</span>
			...
			<section class="room_list room_rank1 active">
				<dl>
					<dd class="flex-space-between" v-for="(item,index) in secondChildren" @click="chooseSubSecond(index)" :key="index">
            <span>{{item}}</span>
            <i class="arrow-right"></i>
          </dd>
				</dl>
    ...
    <div class="chose_layer chose_room" v-show="showChooseThird">
        ...
        <span class="flex1">区</span>
			...
			<section class="room_list room_rank1 active">
				<dl>
					<dd class="flex-space-between" v-for="(item,index) in thirdChildren" @click="chooseThird(index)" :key="item">
            <span>{{item}}</span>
            <i class="arrow-right"></i>
          </dd>
				</dl>
...
export default {
  ...
  methods: {
    getProvinces() {
      this.firstList = [];
      registerService.getProvinces({}).then(res=>{
    ...
    getDistrict(province) {
      this.secondChildren = [];
      registerService.getDistrict({province:province}).then(res=>{
        if(res.status==200&&res.data){
          this.secondChildren=res.data;
    ...
    getAreas(city) {
      this.thirdChildren = [];
      this.basicInfo.cityName = city;
      this.currentChoose.city = city;
      registerService.getAreas({city:city}).then(res=>{
        if(res.status==200&&res.data){
          this.thirdChildren=res.data;
    ...
    chooseHotCity(index){//选择热门城市
      ...
      this.getAreas(hotCityItem.fullName+"市");
    },
    chooseFirst(index) {//选择一级
      let province = this.firstList[index];
      this.basicInfo.provinceName = province;
      this.currentChoose.province = province;
      this.getDistrict(province);//获取二级城市
      console.log(this.basicInfo);
    },
    chooseSubSecond(index) {//选择二级
      let city = this.secondChildren[index];
      this.getAreas(city);
      console.log(this.basicInfo);
    },
    chooseThird(index){//选择三级
      this.currentChoose.area = this.thirdChildren[index];
      this.showChooseThird=false;
